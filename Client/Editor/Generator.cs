    ...
    public static class Generator
    {
        ...
        static void GenOne(Type type, Action<Type, LuaTable> type_info_getter, XLuaTemplate templateAsset, StreamWriter textWriter)
        {
            ...
            try
            {
                ...
                //四、	打包，首先手动注释掉UnityEngineInputWrap.cs文件中用到locationService权限的代码。因为这两行代码会默认开启android获取位置信息的权限，谷歌提审会出现问题。
                // if (genCode.Contains("Utils.RegisterFunc(L, Utils.CLS_GETTER_IDX, \"location\", _g_get_location);"))
                // {
                //     genCode = genCode.Replace("Utils.RegisterFunc(L, Utils.CLS_GETTER_IDX, \"location\", _g_get_location);", "//Utils.RegisterFunc(L, Utils.CLS_GETTER_IDX, \"location\", _g_get_location);");
                //     genCode = genCode.Replace("[MonoPInvokeCallbackAttribute(typeof(LuaCSFunction))]\r\n        static int _g_get_location(RealStatePtr L)\r\n        {\r\n\t\t    try {\r\n                ObjectTranslator translator = ObjectTranslatorPool.Instance.Find(L);\r\n\t\t\t    translator.Push(L, UnityEngine.Input.location);\r\n            } catch(System.Exception gen_e) {\r\n                return LuaAPI.luaL_error(L, \"c# exception:\" + gen_e);\r\n            }\r\n            return 1;\r\n        }",
