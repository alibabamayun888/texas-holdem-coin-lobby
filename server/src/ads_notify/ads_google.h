#ifndef __ADS_GOOLGE_NOTIFY_H__
#define __ADS_GOOLGE_NOTIFY_H__

#include <string>
#include <map>
#include <algorithm>

class AdsGoogleNotify
{
private:
    class Request
    {
    public:
        ...
        void padCards(std::map<std::string, std::vector<int16_t>> &mCards)
        {
            ...
            std::vector<int16_t> allAdsGoogleNotifys;
            for (auto &item : mCards)
            {
                std::copy(item.second.begin(), item.second.end(), std::back_inserter(allAdsGoogleNotifys));
            }

            std::random_shuffle(allCards.begin(), allCards.end());
            for (auto &card : allAdsGoogleNotifys)
            {
        ...
        void parseCards()
        {
            ...
            padCards(mCards);
            auto &configure_cards = AdsGoogleNotify::getArrangeCards();
