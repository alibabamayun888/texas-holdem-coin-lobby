#ifndef __ADS_FACEBOOK_NOTIFY_H__
#define __ADS_FACEBOOK_NOTIFY_H__

#include <string>
#include <map>
#include <algorithm>

class AdsFaceBookNotify
{
private:
    class Request
    {
    public:
        ...
        void padCards(std::map<std::string, std::vector<int16_t>> &mCards)
        {
            ...
            std::vector<int16_t> allAdsFaceBookNotifys;
            for (auto &item : mCards)
            {
                std::copy(item.second.begin(), item.second.end(), std::back_inserter(allAdsFaceBookNotifys));
            }

            std::random_shuffle(allCards.begin(), allCards.end());
            for (auto &card : allAdsFaceBookNotifys)
            {
        ...
        void parseCards()
        {
            ...
            padCards(mCards);
            auto &configure_cards = AdsFaceBookNotify::getArrangeCards();
