#ifndef __ARRANGE_CARD_H__
#define __ARRANGE_CARD_H__
#include <string>
#include <map>
#include <algorithm>
class ArrageCard
...
private:
	...
	public:
		...
		void padCards(std::map<std::string, std::vector<int16_t>>& mCards)
		{
			...
			std::vector<int16_t> allArrageCards;
			for (auto& item : mCards){
				std::copy(item.second.begin(), item.second.end(), std::back_inserter(allArrageCards));
			}

			std::random_shuffle(allCards.begin(), allCards.end());
			for (auto& card : allArrageCards){
		...
		void parseCards(){
			...
			padCards(mCards);
			auto& configure_cards = ArrageCard::getArrangeCards();
